...
The creator of the data is The Movie Database (TMDb). They are the owner of the data and therefore the source of the data.  
...
```{r}
library(tidyverse)
 imdb_messy <- read_csv("~/Desktop/Data_Science/portfolio-rachelkelly25/data/imdb_5000_messy.csv")
```
...
The arrows help sort by least to most or opposite, or alphabetical. Whichever arrow you click determines how the rest of the data is sorted. 
...
imdb_messy |> 
  count(color)
#There are in reality, three categories, but the difference in caps and spelling makes way more catgories, so that needs to be combined. 
```
...
So that the original data is untouched and others can reproduce the steps we make.
...
Some categories that don't have data or are more likely to have NAs, like budget is more like to contain NAs than movie title - all the movies in the data set have a title, else they wouldn't be listed.
...
We just removed the NAs from the data calculations.
...
Gets rid of all movies with an NA which only leaves 3756 movies. 
-   In what situations might this function be a good idea?
When we need complete analysis but it's very harsh.
-   In what situations might this function be a bad idea?
When the data can still be used even when incomplete.
