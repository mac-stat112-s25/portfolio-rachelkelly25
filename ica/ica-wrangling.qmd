...
Yes, we got the same results, no matter the order of select and filter.
...
#___ <- ___ |>
#  ___(___)
#name of wrangled data set <- original data |>
#verb function (variables that you want to keep)
...
#Keeps data from counties in Hawaii and Delaware
```

```{r}
# Keep only data on counties where the Republican got MORE THAN 93.97% of the vote in 2020
# THINK: What variable is relevant here? Republican 2020 data and > 93.97
...
```{r}
# Arrange the counties in elections_small from lowest to highest percentage of 2020 Republican support, automatically places the counties from lowest to highest
...
```{r}
# Arrange the counties in elections_small from highest to lowest percentage of 2020 Republican support, must ad desc() to switch the order
# Print out just the first 6 rows
 elections_small |> 
...
```{r}
# What did this code do? diff_20 is the difference between republican and democratic percentage in 2020
 elections_small |> 
...
```{r}
# What did this code do? repub_votes_20 is the number of votes and not the percentage in 2020
 elections_small |> 
...
```{r}
# What did this code do? repub_win_20 is a true or false column stating whether or not the republican won based on whether more of the vote went to repub or not.
 elections_small |> 
...
Wisconsin data where dems are greater than repubs, arranged highest to lowest
:::
...
-   the results will be the same - this one
-   we'll get an error
...
Is it more effecient to filter first? I'm actually not sure because the results are the same.
...
BEFORE running the below chunk, what do you think it will produce?
Data from Delaware, with a true or false about republicans winning, and county name
...
Select needs to go after all the variables have been defined
...
```{r}
# What does this do? Calculates the median percentage of votes for republicans in 2020 across all counties
 elections_small |> 
   summarize(median(repub_pct_20))
```

```{r}
# What does this do? Same as above but named the variable.
 elections_small |> 
   summarize(median_repub = median(repub_pct_20))
```

```{r}
# What does this do? Same as above but adds total votes to the table
 elections_small |> 
